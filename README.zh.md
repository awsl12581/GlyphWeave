...
- **导出 / 导入** `.gemap` v3 ZIP 格式 — Web 与 Bevy 共用稀疏三维体素世界。
- **小地图** — 实时概览，带视口矩形。点击跳转。
- **视距** — 可配置的渲染边距，让平移更丝滑。
...
渲染 API 将 v3 `.gemap` ZIP 的指定 z 切片转换为图像；兼容窗口内仍接受旧版 JSON。支持三种运行环境：
...
| 生产 (Cloudflare) | `pnpm deploy` | `https://glyphweave.hydroroll.team/api/render` | SVG |
